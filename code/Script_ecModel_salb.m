...
ModelAdapterManager.setDefaultAdapterFromPath(fullfile('C:/Users/apvmo/OneDrive/Escritorio/Estancia_CHALMERS/ec-salb'), 'true'); 
...
saveEcModel(ecModelFull,ModelAdapter,'yml','ec_salbGEMfull');
ecModel=loadEcModel('ec_salbGEMfull.yml');
...
[ecModel, rxnUpdated, notMatch] = applyCustomKcats(ecModel); %%PARA METER DATOS MANUALMENTE (LO IGNORO)
...
[ecModel, rxnsMissingGPR, standardMW, standardKcat] = getStandardKcat(ecModel);  %% IGNORAR
...
ecModel = applyKcatConstraints(ecModel);  %% IGNORAR
...
ecModel = setParam(ecModel,'lb','EX_glc__D_e',-10);
% Run FBA
sol = solveLP(ecModel,1)
saveEcModel(ecModel,ModelAdapter,'yml','ecModel_GEM_salb');
